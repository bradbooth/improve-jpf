...
public class EditorPanel extends ShellPanel {
	...
	protected void initContents(final String fileType, final String fileName) {
		if (fileName != null) {
			...
			if (file.isFile()) {
				...
				/**
				 * // don't block construction of this pane SwingUtilities.invokeLater( new
				 * Runnable(){ public void run(){ warning("file does not exist: " + fileName); }
				 * });
				 **/
			}
		} else {
	...
	/**
	 * override this if content to save needs post processing outside
	 * EventDispatcher
	 */
	...
	public boolean closing() {
		...
		int answer = JOptionPane.showConfirmDialog(getShell(),
